...
echo -e "          ├─ SNI = $DOMAIN        → Nginx HTTP (Komari)"
...
echo -e "真实域名: ${GREEN}$DOMAIN${NC} → Nginx HTTP"
...
sed -i "s/listen 443 ssl/listen $NGINX_PORT ssl/" /etc/nginx/sites-available/$DOMAIN
sed -i "s/listen \[::\]:443 ssl/listen [::]:$NGINX_PORT ssl/" /etc/nginx/sites-available/$DOMAIN

# 3. 配置 Nginx Stream 模块
...
# 4. 修改站点配置支持 proxy_protocol
echo -e "${YELLOW}更新站点配置...${NC}"
...
echo ""
echo -e "${YELLOW}测试:${NC}"
echo -e "访问 Web: ${GREEN}https://$DOMAIN${NC}"
echo -e "VLESS: 使用上面的 Clash 配置"
echo ""
echo -e "${BLUE}工作原理:${NC}"
echo -e "1. 客户端连接 $SERVER_IP:443，SNI=$FAKE_SNI"
echo -e "2. Nginx Stream 识别 SNI != $DOMAIN"
echo -e "3. 转发到 Xray Reality ($REALITY_PORT)"
echo -e "4. 浏览器访问 $DOMAIN → Nginx HTTP → Komari"
echo ""
...
443 → Nginx Stream SNI 分流
     ├─ $DOMAIN → Nginx HTTP:$NGINX_PORT → Komari
