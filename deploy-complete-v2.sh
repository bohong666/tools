...
###############################################################################
# Script Manager 完整部署脚本 v2 (Complete Deployment v2)
# 包含源代码下载、安装和启动
# 用法: bash deploy-complete-v2.sh [domain] [email] [http_port] [https_port] [source_url]
# 示例: bash deploy-complete-v2.sh ss.18avfans.xyz 18avfans@gmail.com 80 8443 https://github.com/your-repo/script-manager/releases/download/v1.0/script-manager-source.tar.gz
###############################################################################
...
if [ $# -lt 1 ]; then
    log_error "缺少必要参数"
    echo "用法: bash deploy-complete-v2.sh <domain> [email] [http_port] [https_port] [source_url]"
    echo "示例: bash deploy-complete-v2.sh ss.18avfans.xyz 18avfans@gmail.com 80 8443"
    exit 1
fi
...
HTTP_PORT=${3:-80}
HTTPS_PORT=${4:-443}
SOURCE_URL=${5:-"https://github.com/bohong666/tools/releases/download/script-manager-v1.0/script-manager-source.tar.gz"}
PROJECT_DIR="/opt/script-manager"
TEMP_DIR="/tmp/script-manager-deploy"

log_info "开始部署 Script Manager (完整版本 v2)"
log_info "域名: $DOMAIN"
log_info "邮箱: $EMAIL"
log_info "HTTP 端口: $HTTP_PORT"
log_info "HTTPS 端口: $HTTPS_PORT"
log_info "源代码 URL: $SOURCE_URL"
...
# 第八步：下载项目源代码
log_info "第八步：下载项目源代码..."
cd $TEMP_DIR
log_info "从 $SOURCE_URL 下载源代码..."

if ! curl -L -o script-manager-source.tar.gz "$SOURCE_URL" 2>/dev/null; then
    log_error "无法下载源代码，请检查 URL 是否正确"
    log_info "您可以手动上传 script-manager-source.tar.gz 到 $TEMP_DIR"
    log_info "然后重新运行此脚本"
    exit 1
fi
...
if [ -d "script-manager" ]; then
    mv script-manager $PROJECT_DIR
else
    log_error "解压失败或目录结构不正确"
    exit 1
fi
log_success "项目已解压到 $PROJECT_DIR"

# 第十步：创建 docker-compose.yml
log_info "第十步：创建 docker-compose.yml..."
cd $PROJECT_DIR
...
# 第十三步：创建 HTTP Basic Auth
log_info "第十三步：创建 HTTP Basic Auth..."
