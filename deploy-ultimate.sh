...
###############################################################################
# Script Manager 终极部署脚本 (Ultimate Version)
# 自动处理 UFW 防火墙、端口冲突、旧容器清理
# 用法: bash deploy-ultimate.sh [domain] [email]
# 示例: bash deploy-ultimate.sh ss.18avfans.xyz 18avfans@gmail.com
###############################################################################
...
if [ $# -lt 1 ]; then
    log_error "缺少必要参数"
    echo "用法: bash deploy-ultimate.sh <domain> [email]"
    echo "示例: bash deploy-ultimate.sh ss.18avfans.xyz 18avfans@gmail.com"
    exit 1
fi
...
log_info "开始部署 Script Manager (终极版本)"
log_info "域名: $DOMAIN"
log_info "邮箱: $EMAIL"
...
check_and_kill_port() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":$port "; then
        log_warning "端口 $port ($port_name) 已被占用，正在释放..."
...
check_and_kill_port 80 "HTTP"
check_and_kill_port 443 "HTTPS"
check_and_kill_port 3306 "MySQL"
check_and_kill_port 3000 "Backend"
check_and_kill_port 5000 "VPS Agent"
