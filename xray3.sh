...
# ============================================
# 【核心修复】兼容新旧版本的密钥解析函数
# Xray v26.x: "Private key: xxx" / "Public key: xxx"  (小写 key，有空格)
# Xray v1.x:  "PrivateKey: xxx"  / "PublicKey: xxx"   (驼峰，无空格)
# ============================================
parse_xray_keys() {
    local key_output="$1"

    # 兼容两种格式解析私钥
    NEW_PRIVATE_KEY=$(echo "$key_output" | grep -iE "^private.?key:" | awk '{print $NF}' | tr -d ' \n\r')

    # 兼容两种格式解析公钥
    NEW_PUBLIC_KEY=$(echo "$key_output" | grep -iE "^public.?key:" | awk '{print $NF}' | tr -d ' \n\r')
...
derive_public_key() {
    ...
    echo "$key_output" | grep -iE "^public.?key:" | awk '{print $NF}' | tr -d ' \n\r'
}
...
update_node_info() {
    ...
    # 【修复】原代码此处错误引用了 $PUBLIC_KEY（旧值），现已使用 $NEW_PUBLIC_KEY
