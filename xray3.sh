...
# 6. 修复官方脚本安装路径不一致导致的密钥生成失败问题 (新增)
# ============================================

SCRIPT_VERSION="v3.3.3-Container-Fix"
...
show_current_info() {
    ...
    # 生成公钥
    KEY_OUTPUT=$("$XRAY_BIN" x25519 -i "$PRIVATE_KEY" 2>&1)
...
generate_uuid() {
    if [ -f /proc/sys/kernel/random/uuid ]; then
        ...
        # 使用xray生成
        "$XRAY_BIN" uuid 2>/dev/null || echo "$(od -x /dev/urandom | head -1 | awk '{OFS="-"; print $2$3,$4,$5,$6,$7$8$9}')"
    fi
}
...
install_xray() {
    ...
    # 安装完成后，更新 XRAY_BIN 全局变量以指向真实的执行文件路径
    XRAY_BIN=$(command -v xray || echo "/usr/local/bin/xray")
    log_info "Xray 路径映射已更新: $XRAY_BIN"
}
...
测试配置: $XRAY_BIN run -test -config /usr/local/etc/xray/config.json
...
3. 验证配置文件: $XRAY_BIN run -test -config /usr/local/etc/xray/config.json
