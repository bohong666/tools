...
###############################################################################
# Script Manager 完整部署脚本 (Complete Version)
# 此脚本包含所有必要的配置文件，无需从 GitHub 克隆
# 用法: bash deploy-complete.sh [domain] [email]
# 示例: bash deploy-complete.sh ss.18avfans.xyz 18avfans@gmail.com
###############################################################################
...
if [ $# -lt 1 ]; then
    log_error "缺少必要参数"
    echo "用法: bash deploy-complete.sh <domain> [email]"
    echo "示例: bash deploy-complete.sh ss.18avfans.xyz 18avfans@gmail.com"
    exit 1
fi
