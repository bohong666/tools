...
###############################################################################
# Script Manager 最终部署脚本 (Final Version)
# 用法: bash deploy-final.sh [domain] [email]
# 示例: bash deploy-final.sh ss.18avfans.xyz 18avfans@gmail.com
###############################################################################
...
if [ $# -lt 1 ]; then
    log_error "缺少必要参数"
    echo "用法: bash deploy-final.sh <domain> [email]"
    echo "示例: bash deploy-final.sh ss.18avfans.xyz 18avfans@gmail.com"
    exit 1
fi
...
# 第六步：生成环境变量（使用 Python 避免 sed 问题）
log_info "第六步：生成环境变量..."
if [ ! -f .env ]; then
    # 生成随机密钥
    ...
    # 使用 Python 创建 .env 文件（避免 sed 特殊字符问题）
    python3 << EOF
import os
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if certbot certonly --standalone -d $DOMAIN -d www.$DOMAIN --non-interactive --agree-tos -m $EMAIL; then
        ...
        # 使用 Python 启用 HTTPS 配置（避免 sed 问题）
        if [ -f nginx.conf ]; then
            python3 << 'PYEOF'
...
content = content.replace('#     ssl_certificate', '    ssl_certificate')
content = content.replace('#     ssl_', '    ssl_')
