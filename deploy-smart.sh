...
###############################################################################
# Script Manager 智能部署脚本 (Smart Version)
# 自动处理端口冲突、清理旧容器、检测系统状态
# 用法: bash deploy-smart.sh [domain] [email]
# 示例: bash deploy-smart.sh ss.18avfans.xyz 18avfans@gmail.com
###############################################################################
...
if [ $# -lt 1 ]; then
    log_error "缺少必要参数"
    echo "用法: bash deploy-smart.sh <domain> [email]"
    echo "示例: bash deploy-smart.sh ss.18avfans.xyz 18avfans@gmail.com"
    exit 1
fi
...
if check_port 3306; then
    log_warning "端口 3306 已被占用"
    PORTS_AVAILABLE=false
fi

if [ "$PORTS_AVAILABLE" = false ]; then
    log_warning "检测到端口冲突，尝试释放占用的进程..."
    
    # 尝试停止占用 80 端口的服务
    if check_port 80; then
        log_info "尝试停止占用 80 端口的服务..."
...
log_info "第五步：安装 Docker Compose..."
...
# 第六步：安装必要工具
log_info "第六步：安装必要工具..."
apt-get install -y -qq apache2-utils git certbot python3-certbot-nginx curl netcat-openbsd psmisc

# 第七步：创建项目目录和配置文件
log_info "第七步：创建项目目录和配置文件..."
mkdir -p $PROJECT_DIR
cd $PROJECT_DIR
...
# 第九步：创建 HTTP Basic Auth
log_info "第九步：创建 HTTP Basic Auth..."
