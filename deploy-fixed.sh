...
###############################################################################
# Script Manager 修复版部署脚本 (Fixed Version)
# 修复 SSL 证书配置和 502 错误问题
# 用法: bash deploy-fixed.sh [domain] [email] [http_port] [https_port]
# 示例: bash deploy-fixed.sh ss.18avfans.xyz 18avfans@gmail.com 80 8443
###############################################################################
...
if [ $# -lt 1 ]; then
    log_error "缺少必要参数"
    echo "用法: bash deploy-fixed.sh <domain> [email] [http_port] [https_port]"
    echo "示例: bash deploy-fixed.sh ss.18avfans.xyz 18avfans@gmail.com 80 8443"
    exit 1
fi
...
log_info "开始部署 Script Manager (修复版本)"
log_info "域名: $DOMAIN"
log_info "邮箱: $EMAIL"
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if docker-compose ps | grep -q "Up"; then
        ...
        if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
            log_warning "服务启动中，等待 10 秒后重试... ($RETRY_COUNT/$MAX_RETRIES)"
            sleep 10
        fi
    fi
done
