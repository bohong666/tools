...
detect_os() {
  ...
  case "${ID:-}" in
    ...
    alpine)        PKG_MGR="apk" ;;
    *) log_err "不支持的发行版：${ID:-unknown}"; exit 1 ;;
  esac
  log_info "检测到系统：$ID"
}
...
install_deps() {
  ...
  if [[ "$PKG_MGR" == "apt" ]]; then
    apt-get update -qq
    apt-get install -y -qq curl wget unzip python3 jq >/dev/null 2>&1 || true
  else
    apk update -q
    apk add --no-cache curl wget unzip python3 jq >/dev/null 2>&1 || true
  fi
}
...
get_ipv4() { curl -s -4 --max-time 6 https://api4.ipify.org || echo ""; }
get_ipv6() { curl -s -6 --max-time 6 https://api6.ipify.org || echo ""; }
...
show_local_node_detail() {
  ...
  echo -e "${CYAN}════════ 本机 Reality 节点：$name ════════${NC}"
  echo "UUID:        $uuid"
  echo "入口端口:    443（对外）"
  echo "本机端口:    $port"
  echo "SNI:         $sni"
...
list_local_nodes() {
  ...
  while IFS='|' read -r uuid port sni priv pub sid name fp || [[ -n "${uuid:-}" ]]; do
    ...
    idx=$((idx+1))
    echo "  [$idx] $name  SNI:$sni  本机端口:$port  UUID:${uuid:0:8}..."
  done < "$LOCAL_DB"
...
add_local_node() {
  ...
  while true; do
    ...
    if [[ "$port" == "443" ]]; then
      log_err "禁止使用 443（443 由 mixed 入口统一管理）"
      continue
    fi
...
show_relay_detail() {
  ...
  echo -e "${CYAN}════════ 中转 Reality 节点：$label ════════${NC}"
  echo "中转 SNI:      $sni"
...
list_relay_targets() {
  echo -e "${CYAN}── 中转 Reality 节点列表 ─────────────────────${NC}"
  if [[ ! -s "$RELAY_DB" ]]; then
    echo "  (无中转节点)"
    return
  fi
...
add_relay_target() {
  log_title "添加中转 Reality 节点（IIJ / 其他 VPS）"
  ...
  read -rp "远程 Reality 服务器 IP（IPv4 或 IPv6）: " rhost
...
delete_local_node() {
  ...
  local line
  line=$(awk -F'|' '!/^#/ && NF>=8' "$LOCAL_DB" | awk -v n="$n" 'NR==n')
...
delete_relay_target() {
  ...
  read -rp "输入要删除的中转节点序号: " n
  n="${n// /}"

  [[ ! "$n" =~ ^[0-9]+$ ]] && { log_err "序号无效"; return; }

  local line
  line=$(awk -F'|' '!/^#/ && NF>=8' "$RELAY_DB" | awk -v n="$n" 'NR==n')
...
main_menu() {
  while true; do
    echo ""
    echo -e "${MAGENTA}===== sing-box v1.13+ VPS 管理器 ${SCRIPT_VERSION} =====${NC}"
    echo -e "${CYAN}1.${NC} 初始化 / 安装 sing-box"
    echo -e "${CYAN}2.${NC} 添加本机 Reality 节点（RFC 落地）"
    echo -e "${CYAN}3.${NC} 添加中转 Reality 节点（IIJ / 其他 VPS）"
    echo -e "${CYAN}4.${NC} 列出本机节点 / 中转节点"
    echo -e "${CYAN}5.${NC} 删除本机节点 / 中转节点"
